define(
    [
          'bx/common/config'
        , 'text!app/views/page/CAPMT/086/_CAPMT086.html'
    ...
    , function (config
		 ...
		 * Backbone
		 */
        var CAPMT086View = Backbone.View.extend({
            // 태그이름 설정
            tagName: 'section'
            // 클래스이름 설정
            , className: 'bx-container CAPMT086-page'
            // 탬플릿 설정
            , templates: {
                'tpl': tpl
            }
            ...
            , initialize: function (initData) {
                ...
                bxProxy.all([
                    ...
                    , {
                        success: function () {
                        that.CAPMT086Grid = new ExtGrid({
                            	 fields: ['jobId', 'prcsngDt', 'rcptNbr','txSeqNbr','atmptCnt', 'exctnNbr','prcsngStsCd','prcsngAmt']
                            , id: 'CAPMT086Grid'
                            , columns: [{
                                text: 'No'
                                , dataIndex: 'rowIndex'
                                ...
                                , {
                                    text: bxMsg('cbb_items.AT#prcsngStsCd')
                                    , flex: 3
                                    , dataIndex: 'prcsngStsCd'
                                    , style: 'text-align:center'
                                   	, align: 'center'
            ...
            , render: function () {
                ...
                // 콤보데이터 로딩
                // 컴포넌트 코드
                fn_getCodeList({className:"CAPMT086-prcsngSts-wrap",targetId:"prcsngStsCd",nullYn:"Y",cdNbr:"A0401"}, this);
            ...
            , resetBase: function () {
            	that = this;
            	that.$el.find('.CAPMT086-base-section [data-form-param="jobId"]').val("");
                that.$('.CAPMT086-base-section [data-form-param="prcsngDt"]').val(getCurrentDate("yyyy-mm-dd"));
            	that.$el.find('.CAPMT086-base-section [data-form-param="rcptNbr"]').val("");
            	that.$el.find('.CAPMT086-base-section [data-form-param="atmptCnt"]').val("");
            	if(that.CAPMT086Grid)
                {
            		that.CAPMT086Grid.resetData();
                }
            }
            ...
            , query: function () {
                ...
                that.pgNbr=1;
                that.totalCnt=0;
                that.sParam = bxUtil.makeParamFromForm($('.CAPMT086-base-section'));
                if(fn_isEmpty(that.sParam.jobId))
                {
                    $('.CAPMT086-base-section [data-form-param="jobId"]').focus();
                    isValid = false;
                }
                else if(fn_isEmpty(that.sParam.prcsngDt))
                {
                    $('.CAPMT086-base-section [data-form-param="prcsngDt"]').focus();
                    isValid = false;
                }
                if(!isValid)
                {
            ...
            , inquiryBaseData: function () {
                ...
                var linkData = {"header": fn_getHeader("PMT0868401"), "inputDto": that.sParam};
                    	 ...
                    	 if (fn_commonChekResult(responseData)) {
                            ...
                            if (tbList != null || tbList.length > 0) {
                                 ...
                                 if (that.sParam.pgNbr == 1) {
                                     // 조회
                                     that.CAPMT086Grid.setData(tbList);
                                     that.rsltCnt=tbList.length;
                                 } else {
                                     // 다음 조회
                                     that.CAPMT086Grid.addData(tbList);
            ...
            , createGrid: function () {
                var that = this;
                this.$el.find(".CAPMT086-grid-section").html(this.CAPMT086Grid.render({'height': '450px'}));
            } // end of createGrid
            /**
             *  작업식별자조회 팝업
             */
            , popupJobId: function () {
            	...
            	popup.on('popUpSetData', function (param) {
            		that.$el.find('.CAPMT086-base-section [data-form-param="jobId"]').val(param.jobId); //작업식별자
            	});
            }
            /**
            ...
            , loadDatePicker: function () {
                fn_makeDatePicker(this.$('.CAPMT086-base-section [data-form-param="prcsngDt"]'));
            }//end of loadDatePicker


            , fn_base_toggle:function()
            {
                fn_pageLayerCtrl(this.$el.find(".CAPMT086-base-section"), this.$el.find("#btn-base-section-toggle"));
            }


            , fn_grid_toggle:function()
            {
                fn_pageLayerCtrl(this.$el.find(".CAPMT086-grid-section"), this.$el.find("#btn-grid-section-toggle"));
            }
