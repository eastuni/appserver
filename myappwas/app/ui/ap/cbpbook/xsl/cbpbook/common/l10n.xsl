...
<xsl:param name="l10n.xml" select="document('../../docbook/common/l10n.xml')"/>
	    ...
	    <xsl:choose>
	      <xsl:when test="$template.node/@text">
	        <xsl:choose>
	          ...
	          <!-- modify:jeh -->
	          <!-- 
	          <xsl:when test="$name = 'section' and $cbp.print.section.title != 0">
	          </xsl:when>
	          -->
	          <xsl:when test="$name = 'table' and $cbp.print.formal.table.title != 0">
	            <xsl:variable name="template.node.text" select="substring-after($template.node/@text,'.')"/>
