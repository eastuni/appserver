...
<xsl:template match="*" mode="class.value">
  ...
  <xsl:if test="starts-with(./@role, 'css.')">
  	<xsl:value-of select="substring-after(./@role,'css.')" />
  </xsl:if> 
</xsl:template>

<xsl:template match="@class" mode="class.value">
  attrivute_<xsl:attribute name="{local-name(.)}">
    <xsl:value-of select="."/>
  </xsl:attribute>
</xsl:template>
...
<xsl:template name="generate.css.headers">
  ...
  <!-- add : jeh -->
  <xsl:call-template name="generate.cbp.css.header"/>
</xsl:template>
...
<!-- add : jeh -->
<xsl:template name="generate.cbp.css.header">

  <xsl:if test="$cbp.css.source != ''">
    <xsl:call-template name="output.cbp.css.header.source">
      <xsl:with-param name="cbp.css.source" select="concat($cbp.css.default.source, $cbp.css.source)"/>
    </xsl:call-template>
  </xsl:if>
  
  <xsl:if test="$cbp.css.source = ''">
    <xsl:call-template name="output.cbp.css.header.source">
      <xsl:with-param name="cbp.css.source" select="$cbp.css.default.source"/>
    </xsl:call-template>
  </xsl:if>  
  
  <xsl:if test="$cbp.css.link.url != ''">
    <xsl:call-template name="output.cbp.css.header.link.url">
      <xsl:with-param name="cbp.css.link.url" select="$cbp.css.link.url"/>
    </xsl:call-template>
  </xsl:if>  
</xsl:template>
...
<!-- add : jeh -->
<xsl:template name="output.cbp.css.header.source">
  <xsl:param name="cbp.css.source"/> 

  <style type="text/css">
    <xsl:copy-of select="$cbp.css.source"/>
  </style>
</xsl:template>

<!-- add : jeh -->
<xsl:template name="output.cbp.css.header.link.url">
  <xsl:param name="cbp.css.link.url"/> 

  <link rel="stylesheet" type="text/css" href="{$cbp.css.link.url}"/>
