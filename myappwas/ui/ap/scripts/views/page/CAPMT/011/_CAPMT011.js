define(
    [
          'bx/common/config'
        , 'text!app/views/page/CAPMT/011/_CAPMT011.html'
    ...
    , function (config
		 ...
		 * Backbone
		 */
        var CAPMT011View = Backbone.View.extend({
            // 태그이름 설정
            tagName: 'section'
            // 클래스이름 설정
            , className: 'bx-container CAPMT011-page'
            // 탬플릿 설정
            , templates: {
                'tpl': tpl
            }
                ...
                bxProxy.all([{
                    ...
                    , {
                        success: function () {
                            that.CAPMT011Grid = new ExtGrid({
                                ...
                                , listeners: {
                                    ...
                                    click: {
                                        element: 'body'
                                        , fn: function () {
                                        	var selectedData = that.CAPMT011Grid.grid.getSelectionModel().selected.items[0];
                                        	if (!selectedData) {
                                        		return;
                                        	}
                            ...
                            // 단일탭 그리드 렌더
                            that.$el.find("#CAPMT011Grid").html(that.CAPMT011Grid.render({'height': CaGridHeight}));
            ...
            , render: function () {
                ...
                // 콤보데이터 로딩
                // 채널코드
                fn_getCodeList({className:"CAPMT011-chnlCd-wrap",targetId:"chnlCd",nullYn:"Y",allNm:bxMsg('cbb_items.SCRNITM#all'),cdNbr:"11930"}, this);
                //에러유무코드
                fn_getCodeList({className:"CAPMT011-errorYnCd-wrap",targetId:"errYn",nullYn:"Y",allNm:bxMsg('cbb_items.SCRNITM#all'),cdNbr:"A0126"}, this);
                //에러레벨코드
                fn_getCodeList({className:"CAPMT011-errorLvlCd-wrap",targetId:"errLvlCd",nullYn:"Y",allNm:bxMsg('cbb_items.SCRNITM#all'),cdNbr:"A0132"}, this);
                //컴포넌트코드
                fn_getCodeList({className:"CAPMT011-cmpntCd-wrap",targetId:"cmpntCd",nullYn:"Y",allNm:bxMsg('cbb_items.SCRNITM#all'),cdNbr:"11602"}, this);
                //상세정보
                // 채널코드
                fn_getCodeList({className:"CAPMT011-chnlCd-wrap2",targetId:"chnlCd",nullYn:"Y",cdNbr:"11930", disabled:true}, this);
                //에러유무코드
                fn_getCodeList({className:"CAPMT011-errorYnCd-wrap2",targetId:"errYn",nullYn:"Y",cdNbr:"A0126", disabled:true}, this);
                //에러레벨코드
                fn_getCodeList({className:"CAPMT011-errorLvlCd-wrap2",targetId:"errLvlCd",nullYn:"Y",cdNbr:"A0132", disabled:true}, this);
                //컴포넌트코드
                fn_getCodeList({className:"CAPMT011-cmpntCd-wrap2",targetId:"cmpntCd",nullYn:"Y",cdNbr:"11602", disabled:true}, this);
                //사용자그룹코드
                fn_getCodeList({className:"CAPMT011-userGrpCd-wrap2",targetId:"userGrpCd",nullYn:"Y",cdNbr:"11001", disabled:true}, this);
            ...
            , resetSearchCondition: function () {
                that = this;
                var d = new Date();
                this.$el.find('#CAPMT011-condition-table [data-form-param="startTxHms"]').val(this.getHms(d, -1));
                this.$el.find('#CAPMT011-condition-table [data-form-param="endTxHms"]').val(this.getHms(d,0));
                this.$el.find('#CAPMT011-condition-table [data-form-param="txDt"]').val(getCurrentDate("yyyy-mm-dd"));
                that.$el.find('#CAPMT011-condition-table [data-form-param="cmpntCd"]').val("");
                that.$el.find('#CAPMT011-condition-table [data-form-param="srvcCd"]').val("");
                that.$el.find('#CAPMT011-condition-table [data-form-param="chnlCd"]').val("");
                that.$el.find('#CAPMT011-condition-table [data-form-param="staffId"]').val("");
                that.$el.find('#CAPMT011-condition-table [data-form-param="errorYnCd"]').val("");
                that.$el.find('#CAPMT011-condition-table [data-form-param="errLvlCd"]').val("");


                if(that.CAPMT011Grid){
                	that.CAPMT011Grid.resetData();
            ...
            , inquireSearchCondition: function () {
                var that = this;
                var sParam = bxUtil.makeParamFromForm($('#CAPMT011-condition-table'));
            ...
            , inquiryList: function (param) {
                ...
                var linkData = {"header": fn_getHeader("CAPMT0118401"), "inputDto": sParam};
                    ...
                    , success: function (responseData) {
                        if (fn_commonChekResult(responseData)) {
                            ...
                            if (tbList != null || tbList.length > 0) {
                                if (sParam.pgNbr == 1) {
                                    // 조회
                                    that.CAPMT011Grid.setData(tbList);
                                    that.$el.find("#searchResultCount").html(bxMsg('cbb_items.SCRNITM#srchRslt')+" ("+fn_setComma(data.totCnt)+" "+bxMsg('cbb_items.SCRNITM#cnt')+")");
                                } else {
                                    // 다음 조회
                                    that.CAPMT011Grid.addData(tbList);
//                                    that.CAPMT011Grid.grid.getSelectionModel().select((that.pgNbr-1) * that.pgCnt + tbList.length -1);
                                }
                                if(tbList.length < that.pgCnt){
                                	that.morePage = false;
                                	that.$el.find("#searchResultCount").html(bxMsg('cbb_items.SCRNITM#srchRslt')+" ("+fn_setComma(that.CAPMT011Grid.getAllData().length)+" "+bxMsg('cbb_items.SCRNITM#cnt')+")");
                                }else{
                                	that.morePage = true;
                                }
                            }
            ...
            ,setTimeInput: function () {
                this.$el.find('#CAPMT011-condition-table [data-form-param="startTxHms"]').mask("99:99:99", {placeholder:"--:--:--"});
                this.$el.find('#CAPMT011-condition-table [data-form-param="endTxHms"]').mask("99:99:99", {placeholder:"--:--:--"});
            }
            /**
            ...
            , inquiryDetail: function (selectedData) {
                ...
                sParam.txDt = sParam.txDt.replace(/-/gi,"");
                sParam.seqNbr=0;
                var linkData = {"header": fn_getHeader("CAPMT0118402"), "inputDto": sParam};
            ...
            , displayDetail: function (detail) {
            	...
            	that.$el.find('#CAPMT011-detail-table [data-form-param="guid"]').val(detail.guid);
            	that.$el.find('#CAPMT011-detail-table [data-form-param="txDt"]').val(txDt);
            	that.$el.find('#CAPMT011-detail-table [data-form-param="txHms"]').val(txHms);
            	that.$el.find('#CAPMT011-detail-table [data-form-param="chnlCd"]').val(detail.chnlCd);
            	that.$el.find('#CAPMT011-detail-table [data-form-param="cmpntCd"]').val(detail.cmpntCd);
                that.$el.find('#CAPMT011-detail-table [data-form-param="srvcCd"]').val(detail.srvcCd);
                that.$el.find('#CAPMT011-detail-table [data-form-param="srvcNm"]').val(detail.srvcNm);
                that.$el.find('#CAPMT011-detail-table [data-form-param="deptId"]').val(detail.deptId);
                that.$el.find('#CAPMT011-detail-table [data-form-param="userGrpCd"]').val(detail.userGrpCd);
                that.$el.find('#CAPMT011-detail-table [data-form-param="staffId"]').val(detail.staffId);
                that.$el.find('#CAPMT011-detail-table [data-form-param="custId"]').val(detail.custId);
                that.$el.find('#CAPMT011-detail-table [data-form-param="errYn"]').val(detail.errYn);
                that.$el.find('#CAPMT011-detail-table [data-form-param="errLvlCd"]').val(detail.errLvlCd);
                that.$el.find('#CAPMT011-detail-table [data-form-param="msgCd"]').val(detail.msgCd);
                var errCntnt = "";
                if(detail.errYn == "Y"){
                	errorCntnt = detail.outpCntnt;
                }
                that.$el.find('#CAPMT011-detail-table [data-form-param="errCntnt"]').val(errCntnt);
                that.$el.find('#CAPMT011-detail-table [data-form-param="dtlMsgCntnt"]').val(detail.dtlMsgCntnt);
                that.$el.find('#CAPMT011-detail-table [data-form-param="sysErrMsgCntnt"]').val(detail.sysErrMsgCntnt);
           ...
           , popupSrvcCd: function () {
               ...
               var that = this;
               var sParam = {};
               sParam.cmpntCd=that.$el.find('.CAPMT011-base-table [data-form-param="cmpntCd"]').val();
...
//                   that.$el.find('[data-form-param="srvcNm"]').val(param.srvcNm); 
           ...
           ,downloadExcelFile: function () {
           	var that = this;
           	that.CAPMT011Grid.exportCsvFile(bxMsg('cbb_items.SCRN#CAPMT011')+"_"+getCurrentDate("yyyy-mm-dd"));
           }
           /**
            *  헤더 탭          
            */
