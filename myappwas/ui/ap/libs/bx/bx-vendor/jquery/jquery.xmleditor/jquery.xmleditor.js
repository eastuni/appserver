...
$.widget( "xml.xmlEditor", {
	...
	constructEditor: function() {
		...
		this.xmlTabContainer = $("<div/>").attr("class", editorTabAreaClass).css("padding-top", (this.editorHeader.height() == 0 ? 40 : this.editorHeader.height()) + "px").appendTo(this.xmlWorkAreaContainer);
	...
	getXml: function() {
		...
		return self.xml2Str(self.xmlState.xml);
	},
	
	// add : jeh
	// get xml string
	setXml: function(xmlString, mode) {
		...
		if (this.textEditor.active) {
			this.textEditor.aceEditor.getSession().setValue(xmlString);			
		}
		
		if (this.guiEditor.isActive) {		
			this.guiEditor.setRootElement(this.xmlState.xml.children()[0], true);
			this.addTopLevelMenu.populate(this.guiEditor.rootElement);
		}
		
		this.xmlState.changesSaved();
	}   
});
...
MenuBar.prototype.generateMenuItem = function(menuItemData, parentMenu) {
	...
	// modify : jeh, hide menu
	if (menuItemData.itemClass == null || menuItemData.itemClass == undefined || menuItemData.itemClass != "header_mode_tab") {		
		menuItem.data("menuItemData", menuItemData).attr("style", "display:none");
	}
