            ...
            {text:bxMsg('ButtonBottomString1'), elCls:'button button-primary write-btn', handler:function(){
            	...
            	if(selectedCondition.conditionAgreeLevel == '02') {
                    ...
                    if (radioType.hasClass('charge-radio-amount')) {
                        if (!PFValidation.minMaxCheck($popupTable, '.type4-min-check', '.type4-max-check', '.type4-default-check', '.type4-increase-check')) {
                    ...
                    // 율
                        ...
                        if (!PFValidation.minMaxCheck($popupTable, '.type5-min-check', '.type5-max-check', '.type5-default-check', '.type5-increase-check')) {
                            PFComponent.showMessage(bxMsg('Z_MinMaxValueError'), 'warning');
                            return;
                        }
            	...
            	// 상품레벨
            		...
            		// 금액
                    if (radioType.hasClass('charge-radio-amount')) {
                    	var checkMsg = PFValidation.minMaxCheckForPfLvl($popupTable, '.type4-min-check', '.type4-max-check', '.type4-default-check', '.type4-increase-check');
                    	if (checkMsg) {
                    ...
                    // 율
                    	...
                    	var checkMsg = PFValidation.minMaxCheckForPfLvl($popupTable, '.type5-min-check', '.type5-max-check', '.type5-default-check', '.type5-increase-check');
                    	if (checkMsg) {
            				PFComponent.showMessage(checkMsg, 'warning');
            				return;
            			}
        ...
        contentsEvent: {
            ...
            'click .isUseSameValueComplexAmount' : function(e) {
                if($(e.currentTarget).find('input').prop('checked')) {
                    $('.cnd-value-type4-popup').find('.type4-max-check').val($('.cnd-value-type4-popup').find('.type4-min-check').val());
                    $('.cnd-value-type4-popup').find('.type4-default-check').val($('.cnd-value-type4-popup').find('.type4-min-check').val());
                }
            },
            'click .isUseSameValueComplexRate' : function(e) {
            	if($(e.currentTarget).find('input').prop('checked')) {
            		$('.cnd-value-type4-popup').find('.type5-max-check').val($('.cnd-value-type4-popup').find('.type5-min-check').val());
            		$('.cnd-value-type4-popup').find('.type5-default-check').val($('.cnd-value-type4-popup').find('.type5-min-check').val());
            	}
            },
            'keyup.xdsoft .type4-min-check' : function(e) {
                if($('.isUseSameValueComplexAmount').find('input').prop('checked')){
                	$('.cnd-value-type4-popup').find('.type4-max-check').val($('.cnd-value-type4-popup').find('.type4-min-check').val());
                	$('.cnd-value-type4-popup').find('.type4-max-check').trigger('blur'); // OHS20180321 - blur처리 소숫점을 동일하게 맞춰주기 위함
                    $('.cnd-value-type4-popup').find('.type4-default-check').val($('.cnd-value-type4-popup').find('.type4-min-check').val());
                    $('.cnd-value-type4-popup').find('.type4-default-check').trigger('blur'); // OHS20180321 - blur처리 소숫점을 동일하게 맞춰주기 위함
                }
            },
            'keyup.xdsoft .type5-min-check' : function(e) {
            	if($('.isUseSameValueComplexRate').find('input').prop('checked')){
            		$('.cnd-value-type4-popup').find('.type5-max-check').val($('.cnd-value-type4-popup').find('.type5-min-check').val());
            		$('.cnd-value-type4-popup').find('.type5-max-check').trigger('blur'); // OHS20180321 - blur처리 소숫점을 동일하게 맞춰주기 위함
            		$('.cnd-value-type4-popup').find('.type5-default-check').val($('.cnd-value-type4-popup').find('.type5-min-check').val());
            		$('.cnd-value-type4-popup').find('.type5-default-check').trigger('blur'); // OHS20180321 - blur처리 소숫점을 동일하게 맞춰주기 위함
            	}
            }
        },
        listeners: {
            afterRenderUI: function() {
                ...
                //renderComboBoxByRate('ProductMeasurementUnitCode', $popup.find('.RtMsurUnitCd'));
